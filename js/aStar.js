...
function aStar(graph,start,goal){
    ...
    while (queue.queue.length) {
        ...
        if (!(processed.has(current))){
            ...
            let neighbourSquares = [[0, 1], [0, -1], [1, 0], [-1, 0]]
            for (let square of neighbourSquares){
                let i = square[0]
                let j = square[1]
                let neighbour = [current[0]+i,current[1]+j]

                if (0 <= neighbour[0] && neighbour[0] < graph.length && 0 <= neighbour[1] && neighbour[1] < graph.length && graph[neighbour[0]][neighbour[1]] != "X"){
                    let newCost = costs[current] + graph[neighbour[0]][neighbour[1]].cost
