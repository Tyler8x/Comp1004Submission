                ...
                for (let j = 0; j < this.dunSize; j++){
                    ...
                    for (let i = 0; i < this.numberOfRooms; i++){
                        ...
                        for (let j = 0; j < MAX_ATTEMPTS; j++){
                            this.pointList.push([(getRandomInt(Math.floor(Number(this.dunSize/5)),Math.floor(Number(this.dunSize*4/5)))),getRandomInt(Math.floor(Number(this.dunSize/5)),Math.floor(Number(this.dunSize*4/5)))])
                            var room = new BasicRoom(this.pointList[this.pointList.length -1],this.dunSize,this.numberOfRooms)
    ...
    MSTCalc(){
        ...
        for (var line of this.allEdges){
            ...
            for (var treeLine of treeLines){
                if (new Edge(line[0],line[1]).isEqualTo(new Edge(treeLine[0],treeLine[1]))){
                    found = true}}
            if (found == true){
        ...
        for (const line of this.AllPaths){
            ...
            for (const item of path){
                if (this.map[item[0]][item[1]].label == "Wall"){
                    this.map[item[0]][item[1]] = new Tile("Door","#AA0000","D",0.3,`Door${this.map[item[0]][item[1]].texture.slice(-1)}`)}
                if (this.map[item[0]][item[1]].label == "Empty"){
                    this.map[item[0]][item[1]] = new Tile("Corridor","#CCAA00","0",0.3,"Corridor")}}}
        for (let i = 0; i < this.dunSize; i++){
            for (let j = 0; j < this.dunSize; j++){
                if (this.map[i][j].label == "Corridor"){
                    for (let index = 0; index < 4; index++){
                        let item = [[1,0],[0,1],[-1,0],[0,-1]][index]
                        if (this.map[i+item[0]][j+item[1]].label == "Empty"){
                            this.map[i+item[0]][j+item[1]] = new Tile('Wall',"#806000","#",8,`Wall${index}`)}}}}}
        return this.map}
    TextureAlign(){
        for (let i = 0; i < this.dunSize; i++){
            for (let j = 0; j < this.dunSize; j++){
                ...
                if (this.map[i][j].label == "Corridor"){
                    for (let temp = 0; temp < 4; temp++){
                        let item = [[1,0],[0,1],[-1,0],[0,-1]][temp]
                        if (this.map[i+item[0]][j+item[1]].label in ["Corridor","Door"]){
                            this.map[i][j].neighbours.push(temp+1)}}
                    if ((this.map[i][j].neighbours.length) == 2){
                        if (getMaxOfArray(this.map[i][j].neighbours) - getMinOfArray(this.map[i][j].neighbours) == 2){
                            this.map[i][j].texture = `CorridorStraight${['Horizontal','Vertical'][min(this.map[i][j].neighbours)-1]}`}
                        else{
                            if (this.map[i][j].neighbours = [1,2]){TurnValue="1"}
                            if (this.map[i][j].neighbours = [2,3]){TurnValue="2"}
                            if (this.map[i][j].neighbours = [3,4]){TurnValue="3"}
                            if (this.map[i][j].neighbours = [1,4]){TurnValue="0"}
                        ...
                        for (let value = 0; value < 5; value++){
                            if (!(value in this.map[i][j].neighbours)){
                                this.map[i][j].texture = `CorridorT${[None,1,2,3,0][value]}`}}}
                    else if ((this.map[i][j].neighbours.length) == 4){
                        this.map[i][j].texture = 'Floor'}}
                if (this.map[i][j].label == "Wall" || this.map[i][j].label == "Corner"){
                    for (let index = 0; index < 4; index++){
                        let item = [[1,0],[0,1],[-1,0],[0,-1]][index]
                        if (["Corridor","Door","Floor"].includes(this.map[i+item[0]][j+item[1]].label)){
                            this.map[i][j].neighbours.push(index+1)}}
                    if (this.map[i][j].neighbours.length == 1){
                        this.map[i][j].texture = `Wall${[null,1,2,3,0][this.map[i][j].neighbours[0]]}`
                        for (let k = 0; k < 2; k++){
                            let position = [[1,1],[-1,1],[-1,-1],[1,-1]][(parseInt(this.map[i][j].texture.slice(-1))+k) % 4]
                            if (i+position[0] < 50 && i+position[0] > 0 && j+position[1] < 50 && j+position[1] > 0){
                                if (["Floor","Corridor"].includes(this.map[i+position[0]][j+position[1]].label)){
                                    this.map[i][j].texture += new String(k)
                                    break}}}}
                    else if (this.map[i][j].neighbours.length == 2){
                        if ((getMaxOfArray(this.map[i][j].neighbours) - getMinOfArray(this.map[i][j].neighbours) == 2)){
                            this.map[i][j].texture = `WallStraight${['Vertical','Horizontal'][getMinOfArray(this.map[i][j].neighbours)-1]}`}
                        else{
                            let TurnValue = ""
                            if ([1,2].includes(this.map[i][j].neighbours[0]) && [1,2].includes(this.map[i][j].neighbours[1])){
                                TurnValue = "1"}
                                if ([2,3].includes(this.map[i][j].neighbours[0]) && [2,3].includes(this.map[i][j].neighbours[1])){
                                TurnValue = "2"}
                                if ([3,4].includes(this.map[i][j].neighbours[0]) && [3,4].includes(this.map[i][j].neighbours[1])){
                                TurnValue = "3"}
                                if ([1,4].includes(this.map[i][j].neighbours[0]) && [1,4].includes(this.map[i][j].neighbours[1])){
                                TurnValue = "0"}
                            this.map[i][j].texture = `WallTurn${TurnValue}`
                            let position = [[-1,1],[-1,-1],[1,-1],[1,1]][parseInt(this.map[i][j].texture.slice(-1))]
                            if (i+position[0] < 50 && i+position[0] > 0 && j+position[1] < 50 && j+position[1] > 0){
                                if (["Floor","Corridor"].includes(this.map[i+position[0]][j+position[1]].label)){
                    ...
                    for (let k = -1; k < 2; k++){
                        for (let l = -1; l < 2; l++){
                            if (i+k+1 < this.dunSize && j+l+1 < this.dunSize){
                                if (i+k >= 0 && j+l >= 0){
                                    if (["Floor","Corridor"].includes(this.map[i+k][j+l].label)){
                                        this.map[i][j].neighbours.push([k,l])}}}}}
                    if (this.map[i][j].neighbours.length == 1){
                        let textureValue = ""
                        if (this.map[i][j].neighbours[0][0] ==  1 && this.map[i][j].neighbours[0][1] == 1){textureValue="Corner0"}
                        if (this.map[i][j].neighbours[0][0] ==  -1 && this.map[i][j].neighbours[0][1] == 1){textureValue="Corner1"}
                        if (this.map[i][j].neighbours[0][0] ==  1 && this.map[i][j].neighbours[0][1] == -1){textureValue="Corner2"}
                        if (this.map[i][j].neighbours[0][0] ==  -1 && this.map[i][j].neighbours[0][1] == -1){textureValue="Corner3"}
                        this.map[i][j] = new Tile("Corner","#806000","X",99,`${textureValue}`)}}}}}
    output(){
...
function Debug(){
    ...
    UpdateInputs()
    Grid = new Dungeon(dunSize=dunSize,rooms=RoomNumber,tileSize=textureSize,corridorChance=corridorChance)
    Grid.output()
}