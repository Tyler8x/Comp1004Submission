class Triangle{
    constructor(point1,point2,point3){
        ...
        //try {
            this.circumcentre = this.circumcentreCalc(this.p1,this.p2,this.p3)}      //Must be in a try/except, if all points lay on a line the circumcentre
    ...
    circumcentreCalc(a, b, c){
        ...
        return [1 / D * (ad * (b[1] - c[1]) + bd * (c[1] - a[1]) + cd * (a[1] - b[1])), //#Calculates the circumcentre of the triangle
...
function Triangulate(pointList){
    ...
    for (let point of pointList){
        ...
        for (let triangle of triangulation){
            if (Math.pow(triangle.p1[0] - triangle.circumcentre[0],2)+Math.pow(triangle.p1[1]-triangle.circumcentre[1],2) > Math.pow(point[0] - triangle.circumcentre[0],2)+Math.pow(point[1]-triangle.circumcentre[1],2)){
        ...
        for (let edge of polygon) {
            ...
            function pointInSuperTriangle (triangle) {
                let a = [SupTriPoint1,SupTriPoint2,SupTriPoint3].includes(triangle.p1)
                let b = [SupTriPoint1,SupTriPoint2,SupTriPoint3].includes(triangle.p2)
                let c = [SupTriPoint1,SupTriPoint2,SupTriPoint3].includes(triangle.p3)
                return  a || b || c
            }
